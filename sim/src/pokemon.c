...
static u8 sLearningMoveTableID;
...
// simulator: species-level TM/HM learnset check (CanMonLearnTMHM needs a struct Pokemon)
u32 SpeciesCanLearnTMHM(u16 species, u8 tm)
{
    if (species == SPECIES_EGG)
        return 0;
    if (tm < 32)
        return sTMHMLearnsets[species][0] & (1u << tm);
    return sTMHMLearnsets[species][1] & (1u << (tm - 32));
}
