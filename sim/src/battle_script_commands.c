...
// The game's table has 4 entries but protectUses is a u8 that keeps counting: from the 5th consecutive
// Protect/Detect/Endure on, the game indexes past the table into the data that follows it in ROM (the
// Mimic and Metronome forbidden-move lists, the Flail table, ...). The simulator reproduces that exactly with
// the 256 halfwords starting at the table in the retail ROM (0x082507E0; the harness ROM has the same bytes).
static const u16 sProtectSuccessRates[256] = {
    0xFFFF, 0x7FFF, 0x3FFF, 0x1FFF, 0x0076, 0x00A5, 0x00A6, 0x0066,
    0xFFFE, 0x0044, 0x00F3, 0x00B6, 0x00C5, 0x00CB, 0x00C2, 0x00D6,
    0x00A8, 0x010A, 0x0121, 0x010E, 0x0157, 0x010F, 0x0108, 0xFFFF,
    0xC801, 0x9604, 0x6409, 0x5010, 0x2820, 0x1430, 0x004E, 0x004B,
    0x0059, 0x0038, 0x0039, 0x003D, 0x009D, 0x00F7, 0x0081, 0x0081,
    0x0064, 0x0014, 0x00FA, 0x0028, 0x01F4, 0x003C, 0x03E8, 0x0050,
    0x07D0, 0x0064, 0xFFFF, 0xFFFF, 0x008B, 0x000F, 0x0085, 0x0019,
    0x0086, 0x0023, 0x0087, 0x002D, 0x0088, 0x0037, 0x0089, 0x0041,
    0x008C, 0x004B, 0x012A, 0x0050, 0x0045, 0x0055, 0x0044, 0x005A,
    0x006E, 0x005F, 0x00A3, 0x0060, 0x00A4, 0x0061, 0x00A5, 0x0062,
    0x00A6, 0x0063, 0x00A7, 0x0001, 0x0C0C, 0x0B04, 0x0B0B, 0x0505,
    0x0000, 0x0F14, 0x0F0A, 0x0000, 0xAE60, 0xFF7E, 0x0B91, 0x0803,
    0x13B1, 0x0803, 0x1439, 0x0803, 0x1E8D, 0x0803, 0x1F01, 0x0803,
    0x1F69, 0x0803, 0x2161, 0x0803, 0x227D, 0x0803, 0x2429, 0x0803,
    0x2591, 0x0803, 0x2651, 0x0803, 0x273D, 0x0803, 0x275D, 0x0803,
    0x27B1, 0x0803, 0x2811, 0x0803, 0x2841, 0x0803, 0x2AFD, 0x0803,
    0x2B71, 0x0803, 0x2BD5, 0x0803, 0x2C49, 0x0803, 0x2C91, 0x0803,
    0x2CED, 0x0803, 0x2D51, 0x0803, 0x2E29, 0x0803, 0x2E4D, 0x0803,
    0x2F4D, 0x0803, 0x2FE9, 0x0803, 0x3061, 0x0803, 0x30C9, 0x0803,
    0x3135, 0x0803, 0x3141, 0x0803, 0x31F5, 0x0803, 0x3225, 0x0803,
    0x3231, 0x0803, 0x3245, 0x0803, 0x3259, 0x0803, 0x326D, 0x0803,
    0x3281, 0x0803, 0x329D, 0x0803, 0x32D5, 0x0803, 0x32ED, 0x0803,
    0x3315, 0x0803, 0x3385, 0x0803, 0x3391, 0x0803, 0x33D5, 0x0803,
    0x3405, 0x0803, 0x3445, 0x0803, 0x3479, 0x0803, 0x376D, 0x0803,
    0x3879, 0x0803, 0x38C9, 0x0803, 0x38ED, 0x0803, 0x394D, 0x0803,
    0x39B5, 0x0803, 0x3A11, 0x0803, 0x3A79, 0x0803, 0x3AC5, 0x0803,
    0x0200, 0x0103, 0x4848, 0x5A20, 0x5050, 0x5850, 0x162C, 0x08D0,
    0x0180, 0xD6EB, 0x170C, 0x08D0, 0xD6EB, 0x0000, 0x0000, 0x4000,
    0x0400, 0x0000, 0x0000, 0x0004, 0x0004, 0x0004, 0x0008, 0x0004,
    0xFFFF, 0x0000, 0x0000, 0x0044, 0x0004, 0x0044, 0x0008, 0x0044,
    0xFFFF, 0x0000, 0x0000, 0x0084, 0x0004, 0x0084, 0x0008, 0x0084,
    0xFFFF, 0x0000, 0x0000, 0x00C4, 0x0004, 0x00C4, 0x0008, 0x00C4,
};
...
static void Cmd_tryfaintmon(void)
{
    ...
    else
    {
         ...
         && gBattleMons[gActiveBattler].hp == 0)
        {
            ...
            // simulator: the game reads gStatuses3[gBattlerTarget] even when gBattlerTarget == 4 (no target),
            // which in EWRAM is the first word of gDisableStructs[0] (transformedMonPersonality); reproduce that.
            if (((gBattlerTarget < MAX_BATTLERS_COUNT) ? gStatuses3[gBattlerTarget] : gDisableStructs[0].transformedMonPersonality) & STATUS3_GRUDGE
...
static void Cmd_playanimation(void)
{
    ...
    {
        BtlController_EmitBattleAnimation(BUFFER_A, gBattlescriptCurrInstr[2], argumentPtr ? *argumentPtr : 0);
        MarkBattlerForControllerExec(gActiveBattler);
        gBattlescriptCurrInstr += 7;
    }
...
static void Cmd_playanimation_var(void)
{
    ...
    {
        BtlController_EmitBattleAnimation(BUFFER_A, *animationIdPtr, argumentPtr ? *argumentPtr : 0);
        MarkBattlerForControllerExec(gActiveBattler);
        gBattlescriptCurrInstr += 10;
    }
    else if (gHitMarker & HITMARKER_NO_ANIMATIONS)
...
static void Cmd_getmoneyreward(void)
{
    ...
    {
        if (gTrainerBattleOpponent_A == TRAINER_SECRET_BASE || gTrainers[gTrainerBattleOpponent_A].partySize == 0)
        {
            moneyReward = 0; // secret bases do not exist in FRLG; partySize 0 => ad-hoc simulator party
        }
        else
        {
...
static void Cmd_hpthresholds2(void)
{
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_DOUBLE))
    {
        ...
        // simulator: guard the division (a zero switch-out HP cannot happen in the game; the host would trap)
        result = hpSwitchout ? (hpSwitchout - gBattleMons[opposingBattler].hp) * 100 / hpSwitchout : 0;
...
// Conversion 2
// simulator: the original loop can index 3 bytes past gTypeEffectiveness (i == 336); these are the
// bytes that follow the table in the retail ROM, so the behaviour (never matching) is reproduced exactly.
static const u8 sTypeEffectivenessRomOverflow[3] = { 0xC8, 0xC9, 0xCC };
#define TE_ROM(idx) ((idx) < (s32)sizeof(gTypeEffectiveness) ? gTypeEffectiveness[idx] : sTypeEffectivenessRomOverflow[(idx) - sizeof(gTypeEffectiveness)])
...
static void Cmd_settypetorandomresistance(void)
{
    ...
    else
    {
        ...
        for (rands = 0; rands < 1000; rands++)
        {
            ...
            if (TE_ROM(i + 0) == gLastHitByType[gBattlerAttacker]
                && TE_ROM(i + 2) <= TYPE_MUL_NOT_EFFECTIVE
                && !IS_BATTLER_OF_TYPE(gBattlerAttacker, TE_ROM(i + 1)))
            {
                SET_BATTLER_TYPE(gBattlerAttacker, TE_ROM(i + 1));
                PREPARE_TYPE_BUFFER(gBattleTextBuff1, TE_ROM(i + 1));
