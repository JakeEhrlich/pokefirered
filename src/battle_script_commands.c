        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP))
                {
                    ...
                    // Level cap: never gain exp past the cap level.
                    {
                        u32 curExp = GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_EXP);
                        u32 capExp = GetLevelCapExp(GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_SPECIES));
                        if (curExp + gBattleMoveDamage > capExp)
                            gBattleMoveDamage = capExp - curExp;
                    }
